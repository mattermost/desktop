...
// Windows' built-in IPv6 transition adapters report IF_TYPE_TUNNEL but are not VPNs.
const TRANSITION_ADAPTER_PATTERN = /6to4|isatap|teredo/i;
...
export class WindowsSessionAttributeCollector extends SessionAttributeCollector {
    ...
    private isVpnAdapter(adapter: AdapterInfo): boolean {
        if (TRANSITION_ADAPTER_PATTERN.test(adapter.friendlyName) || TRANSITION_ADAPTER_PATTERN.test(adapter.description)) {
            return false;
        }
