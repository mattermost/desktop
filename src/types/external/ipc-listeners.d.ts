...
declare namespace Electron {
    export interface IpcRenderer {
        on(event: 'user-activity-update', listener: (event: IpcMainEvent, status: {
            userIsActive: boolean;
            idleTime: number;
            isSystemEvent: boolean;
        }) => void): this;
        on(event: 'notification-clicked', listener: (event: IpcMainEvent, info: {
            channel: {id: string};
            teamId: string;
            url: string;
        }) => void): this;
        on(event: 'browser-history-status-updated', listener: (event: IpcMainEvent, status: {
            canGoBack: boolean;
