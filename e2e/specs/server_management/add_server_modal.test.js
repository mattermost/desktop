...
describe('Add Server Modal', function desc() {
    ...
    describe('Pre-Auth Header Functionality', () => {
        beforeEach(async () => {
            await newServerView.type('#serverUrlInput', 'http://example.org');
            await newServerView.type('#serverNameInput', 'TestServer');
        });
        ...
        it('should validate server with pre-auth secret', async () => {
            await newServerView.route('**/api/v4/system/ping', (route) => {
                if (route.request().headers()['x-mattermost-preauth-secret']) {
                    route.fulfill({
                        status: 200,
        ...
        it('should save server with pre-auth secret', async () => {
            await newServerView.route('**/api/v4/system/ping', (route) => {
                route.fulfill({
                    status: 200,
                    contentType: 'application/json',
